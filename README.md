...
> 아래 기능은 목표 제품 기준이며, 현재 MVP는 Rule·Statistical 2계층 탐지까지 구현되어 있습니다.
...
3계층 앙상블이 이상을 탐지한 이후, 사용자 본인의 매매 데이터를 근거로 분석 리포트를 제공합니다. 리포트는 다음 세 가지 컨텍스트를 교차하여 구성됩니다.
...
* **Synthetic Data:** 실제 시세 위에서 **자본시장연구원 실증 보고서([이슈보고서 22-02](https://www.kcmi.re.kr/report/report_view?report_no=1481), [이슈보고서 21-11](https://www.kcmi.re.kr/report/report_view?report_no=1243))의 공표 통계를 재현하도록 캘리브레이션한 가상 투자자 행동 시뮬레이션**으로, 실제와 최대한 유사한 학습 데이터를 자체 제작합니다. 상세: [synthetic_data.md](synthetic_data.md)
...
│
├── synthetic_data/   # 합성 학습 데이터 생성기 (가상 투자자 행동 시뮬레이션 + 피처 빌더)
