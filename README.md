...
```
> 로컬 실행 시 SQLite DB(`local.db`)가 자동 생성됩니다. 별도 DB 설치가 필요 없습니다.
...
# 백엔드 주소 지정: frontend/.env 파일 생성 후 아래 한 줄 추가
...
### 3. (선택) DB 초기화
업로드된 거래·분석 데이터를 비울 때 사용합니다.
```bash
cd backend
python reset_db.py      # 로컬 SQLite 초기화 (확인 후 yes 입력)
```
> 로컬 DB(SQLite)의 거래·분석 데이터를 비웁니다. 확인 메시지에 yes를 입력해야 실행됩니다.
...
설치 없이 배포된 버전에서 바로 체험할 수 있습니다.

**데모 시나리오**
1. **업로드** — 기존 매매내역 CSV 업로드 → 전체 거래 분석
2. **업로드** — 신규 매매내역 CSV 업로드 → 이전에 없던 거래만 추가 분석
3. **대시보드** — 위험 점수 · 투자 성향 프로파일 확인
4. **리포트** — AI 기반 매매 행동 코칭 리포트 확인
