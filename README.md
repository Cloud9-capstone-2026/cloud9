...
2. **업로드** — 기존 매매내역 CSV 업로드 → 전체 거래 분석
3. **업로드** — 신규 매매내역 CSV 업로드 → 이전에 없던 거래만 추가 분석
4. **대시보드** — 위험 점수 · 투자 성향 프로파일 확인
5. **리포트** — AI 기반 매매 행동 코칭 리포트 확인
...
- **업로드 & 분석** — 매매내역 CSV를 업로드하면 이전에 없던 신규 거래를 분석합니다.
- **대시보드** — 위험 점수와 투자 성향 프로파일을 확인합니다.
- **리포트** — AI 기반 매매 행동 코칭 리포트를 확인합니다.
