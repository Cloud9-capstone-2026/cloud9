...
"""
pipeline/jobs.py의 기존 _store_trades 함수를 아래로 교체.

변경 사항 (JWT 도입에 따른 필수 수정):
1. CsvUpload.user_id를 조회해서 Trade.user_id에 반영 — 이게 없으면
   trades.py의 "본인 거래만 조회" 필터에 걸려서 업로드한 거래가
   화면에 하나도 안 보이는 버그가 생긴다.
2. 중복 체크(exists 쿼리)에 Trade.user_id 조건 추가 — 기존 코드는
   전체 유저를 통틀어 5컬럼 일치만 봤기 때문에, 다른 유저가 우연히
   동일한 거래(같은 날짜/종목/구분/수량/단가)를 넣으면 "중복"으로
   오인되어 저장 자체가 스킵되는 문제가 있었다 (다인원 서비스 전환 시
   반드시 고쳐야 하는 부분).
"""
...
def _store_trades(db, upload_id: int) -> None:
    ...
    중복 체크는 사용자 범위 내에서 5컬럼(거래일자·종목명·거래구분·거래수량·
    거래단가) — 증권사 CSV에는 정산금액이 없어 기본값으로 채워지므로 중복
    키에 넣으면 판정이 왜곡되고, 분석(detect)의 신규 거래 추출 키와도 이
    5컬럼이 일치한다.
    """
