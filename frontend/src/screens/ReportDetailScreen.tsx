        ...
        <View style={{ gap: 13 }}>
          <Card style={styles.resultCard}>
            ...
            <View style={styles.layersRow}>
              {d.layers.map((l) => (
                <View key={l.label} style={styles.layerItem}>
                  ...
                  <Text style={{ fontSize: 12, color: l.triggered ? C.navy : C.muted, fontWeight: l.triggered ? '600' : '400' }}>
                    {l.label}
                  </Text>
                </View>
              ))}
            ...
            {d.lstmFailed && (
              <View style={styles.lstmFailNote}>
                <Text style={{ fontSize: 10, color: C.muted }}>딥러닝 계층 분석 실패</Text>
              </View>
            )}
          </Card>
          ...
          <View style={styles.twoColRow}>
            <Card style={styles.twoColCard}>
              <Text style={styles.smallCardTitle}>탐지된 규칙</Text>
              {d.rules.map((r) => (
                ...
                <View key={b.key}>
                  <View style={styles.biasHeaderRow}>
                    <View style={styles.biasHeaderLeft}>
                      <Text style={{ fontSize: 13, fontWeight: b.isTop ? '600' : '400', color: b.isTop ? C.navy : C.muted }}>
                        {b.name}
                      </Text>
                      {b.isTop && (
                    ...
                    <Text style={{ fontSize: 13, fontWeight: '600', color: b.color }}>{b.score}</Text>
                  </View>
                ...
                return (
                  <View key={e.key}>
                    <View style={styles.evNameRow}>
                      ...
                      <Text style={{ fontSize: 13, color: C.navy, fontWeight: e.isTop ? '600' : '500' }}>{e.name}</Text>
                    </View>
                    ...
                    >
                      <Text style={{ fontSize: 12, color: C.navy }}>영향을 준 요인 보기</Text>
...
const styles = StyleSheet.create({
  ...
  stockName: { fontSize: 24, fontWeight: '700', color: C.navy, letterSpacing: -0.5 },
  dateText: { fontSize: 12, color: C.muted },
  sectionTitle: { fontSize: 18, fontWeight: '600', color: C.navy, letterSpacing: -0.1, paddingHorizontal: 2, marginBottom: 10 },
  detailGrid: { flexDirection: 'row', flexWrap: 'wrap', rowGap: 14 },
  detailCell: { width: '50%' },
  detailLabel: { fontSize: 11, color: C.muted, marginBottom: 3 },
  detailValue: { fontSize: 14, fontWeight: '500', color: C.navy },
  resultCard: { paddingVertical: 14 },
  layerSummary: { fontSize: 12, color: C.muted, marginBottom: 4 },
  verdict: { fontSize: 22, fontWeight: '600', letterSpacing: -0.5 },
  ...
  smallCardTitle: { fontSize: 12, fontWeight: '500', color: C.muted, marginBottom: 10 },
  ruleText: { fontSize: 16, fontWeight: '700', color: '#111', lineHeight: 26 },
  deviationCard: { justifyContent: 'space-between' },
  devLabel: { fontSize: 15, fontWeight: '700', color: '#111', lineHeight: 20, marginBottom: 4 },
  sigmaText: { fontSize: 13, fontWeight: '500', color: C.muted },
  ...
  gaugeLabel: { flex: 1, textAlign: 'center', fontSize: 9 },
  ...
  topBadgeText: { fontSize: 10, fontWeight: '500', color: C.blue },
  biasTrack: { height: 5, backgroundColor: C.mutedBg, borderRadius: 999, overflow: 'hidden' },
  biasFill: { height: '100%', borderRadius: 999 },
  evidenceDesc: { fontSize: 12, color: C.muted, lineHeight: 19, marginBottom: 14 },
  ...
  evShareLabel: { fontSize: 10, color: C.muted },
  ...
  evRank: { fontSize: 11, fontWeight: '600', color: C.muted, width: 14, textAlign: 'right' },
  evFeatureName: { fontSize: 12, color: C.navy, flex: 1 },
  evDir: { fontSize: 11, fontWeight: '600' },
  ...
  dartBadgeText: { fontSize: 10, color: C.muted },
  newsCaution: { fontSize: 11, color: C.muted, marginBottom: 10, marginTop: 2, paddingHorizontal: 2, lineHeight: 18 },
});
